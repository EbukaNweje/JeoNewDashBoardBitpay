...
const DepositPay = () => {
  ...
  const handlegetallWalletAddress = async () => {
    await axios
      .get("https://new-joe-back2025.vercel.app/api/getallWalletAddress")
      .then((response) => {
  ...
  const url = `https://new-joe-back2025.vercel.app/api/sendpayment/${id}`;
  const url2 = `https://new-joe-back2025.vercel.app/api/deposit/${id}
`;
  const data = {
  ...
  return (
    <div className="w-full h-max flex bg-[#f5f6fa] px-48 phone:gap-6 phone:px-6 phone:flex-col py-4">
      <div className="w-full h-max flex flex-col items-center gap-2">
        <div
          className="w-full h-max flex items-center gap-2 text-lg cursor-pointer"
          onClick={() => Nav(-1)}
        >
          <FaArrowLeftLong />
          <p>Back</p>
        ...
        <div className="w-[45%] phone:w-full h-max flex flex-col bg-white mt-4 rounded border-gray-300 border">
          ...
          <div className="w-full h-max rounded-b p-4">
            <p>
              Note: click paid if have made payment to the above wallet and wait
              for the system to confirm your deposit!
            </p>
          </div>
        </div>
      </div>
